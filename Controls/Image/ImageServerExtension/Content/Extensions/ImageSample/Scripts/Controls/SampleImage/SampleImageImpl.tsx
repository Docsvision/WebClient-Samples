namespace WebClient.LayoutExtension {
    export interface SampleImageImplState extends BaseControlImplState, SampleImageState {
    }

    export class SampleImageImpl extends BaseControlImpl<SampleImageParams, SampleImageImplState> {
        constructor(props:  SampleImageParams) {
            super(props);
        ...
        protected getCssStyle(): React.CSSProperties {
            ...
            if (this.state.imageWidth) {
                // BaseControl.getCssStyle возвращает this.props.customCssStyle (который может быть настроен через скрипты)
                // Вызов newValueIfUndefined означает, что мы задаем значение width только в том случае, 
                // если оно не было задано this.props.customCssStyle (чтобы не затереть значение, которое задано через скрипты)

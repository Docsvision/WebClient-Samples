namespace WebClient {
        ...
        attachRootNode = (elem: HTMLElement) => {
            if (elem) {
                let script = window["HC_LOAD_INIT"]
                    ? workScriptTemplate.format(this.props.widgetId, this.layout.cardInfo.id)
                    : initializeScriptTemplate.format(this.props.widgetId, this.layout.cardInfo.id);
    ...
    // Регистрируем контрол под именем HyperComments в controlFactory
    controlFactory.register("HyperComments", () => HyperComments);
}
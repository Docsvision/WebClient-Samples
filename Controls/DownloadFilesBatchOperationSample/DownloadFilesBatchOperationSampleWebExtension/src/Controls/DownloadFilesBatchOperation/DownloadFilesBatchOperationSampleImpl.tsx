...
import { DownloadFilesBatchOperationSampleParams } from './DownloadFilesBatchOperationSample';
 ...
 */
export class DownloadFilesBatchOperationSampleImpl extends BaseControlImpl<DownloadFilesBatchOperationSampleParams, DownloadFilesBatchOperationSampleState> {
    ...
    private onDownloadFilesClick = async () => {
        ...
        let downloadFilesTask = async (row: ITableRowModel) => {
            ...
            try {
                let url = this.state.services.urlResolver.resolveApiUrl("GetDocumentFiles", "DownloadFilesBatchOperationSampleSample");
                ...
                for (let fileInfo of fileInfos) {
                    if (this.state.downloadDocumentFileMode == DownloadDocumentFileMode.All ||
                        ...
                        FileSaver.saveAs(fileData, fileInfo.fileName);
                    }
                }
    ...
    renderControl() {
        ...
        return (
            <Button stretchWidth={false} onClick={this.onDownloadFilesClick} disabled={!this.isAvailable()} title={tooltip} >
                {this.state.buttonText ?? resources.DownloadFilesBatchOperation}
            </Button>
        );
    }
}
...
declare namespace styled {
    ...
    export interface StyledComponentClass<P, T, O = P> extends React.ComponentClass<ThemedOuterStyledProps<O, T>> {
        ...
        withComponent<K extends keyof SVGTags>(tag: K): StyledComponentClass<React.SVGProps<SVGTags[K]>, T, O>;
        withComponent<TargetP>(element: Component<TargetP>): StyledComponentClass<P, T, TargetP & O>;
    }
    ...
    export interface ThemedBaseStyledInterface<T> extends ThemedStyledComponentFactories<T> {
        ...
        <P extends { [prop: string]: any }>(component: React.ComponentType<P>): ThemedStyledFunction<P, T, WithOptionalTheme<P, T>>;
    }
    ...
    type WithOptionalTheme<P, T> = Pick<P, Exclude<keyof P, "theme">> & { theme?: T; };

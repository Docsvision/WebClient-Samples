...
export async function sampleDocument_loadPartnersInfo(services: $SamplePartnersController & $ControlStore) {
    ...
    if (departmentIds.length > 0) {
        ...
        departmentIds.forEach(function (departmentId, idIndex) {
            let partnersControls = controls.samplePartnersDepartment && samplePartnersDepartmentControls.filter(function (control) {
                return control.hasValue() && control.params.value.id === departmentId;
            });
